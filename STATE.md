...
| 12 | Incentive: port Panso period picker + ‹/› steppers | built | (see COMMIT 1/2) | — | [A1] pp-tabs + pp-cluster + stepPeriod() ported verbatim from Panso (markup/CSS/JS). Picker delimited for mata-hr IAM reuse. 41938→51442 B. Parse OK. ⚠ mata-hr: intern-allowance-matrix.html needs the identical picker block. |
| 13 | Incentive: make incentive column visually distinct | built | (see COMMIT 1/2) | — | [A2] .ic-inc-col (green left-border + #f0fdf4 bg) on TH/TD body/TD footer; mobile .ic-card-incentive enhanced. Styling only — no formula change. +672 B. Parse OK. |
| 14 | Incentive: persistent "Data as of" freshness indicator | built | (see COMMIT 1/2 + 2/2) | — | [A3] Header chip (.data-freshness-chip, id="data-freshness") populated from periodsData.data_synced_at (DATA_DIR mtime, UTC). tools_api.py list_available_periods() returns data_synced_at. Results-footer data_as_of unchanged. Parse OK / py_compile OK. |
| 15 | Dept Documents: single-source sync w/ Panso (D11) | built | (see COMMIT 2/2) | — | **D11 RESOLVED: read-only single shared bucket.** Panso writes gs://panso-ph-data/data/dept-documents/ (R/W FUSE); Mata reads same path (readonly=true FUSE, cloudbuild.yaml L25-26). Drift impossible by construction. docs_dir() gains D11 comment + mkdir try/except guard. dept-documents.html: "Synced from department's Panso documents." UI hint added. py_compile OK, Parse OK. |
...
**UI-feedback round (Sonnet 2026-06-27): Tasks 12–15 built. Awaiting Ryan deploy + Opus QA.**
