...
| 08 | Repo hygiene (.gitattributes + housekeeping) | pending | — | — | Runnable now. Repo-only, no deploy. |
| 09 | Port Dept Documents (read-only) | pending | — | — | Runnable now. New `dept-documents.html` + `GET /api/dept/documents`. D11 read-only. |
